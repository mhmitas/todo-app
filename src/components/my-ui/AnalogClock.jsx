        ...
        <div className='flex justify-center mb-4 overflow-auto'>
            <div className='size-max rounded-full p-2 bg-gradient-to-b from-amber-900 via-gray-500 to-gray-600'>
                <div className="relative flex items-center justify-center bg-white rounded-full size-64">
                    {/* Clock Center */}
                    <div className="absolute size-5 bg-gray-800 rounded-full z-[4]"></div>

                    {/* Hour Hand */}
                    <div className="absolute flex w-3 h-28 top-4 origin-bottom z-[1] rounded-full" style={{ transform: `rotate(${((hour * 30) + ((minute / 60) * 30))}deg)` }}>
                        <div className='flex-1 w-full mt-10 bg-gray-600 rounded-full'></div>
                    </div>
                    {/* Minute Hand */}
                    <div className="absolute flex w-2 h-28 top-4 origin-bottom z-[2] rounded-full" style={{ transform: `rotate(${minute * 6}deg)` }}>
                        <div className='flex-1 w-full mt-5 bg-blue-600 rounded-full'></div>
                    </div>
                    {/* Second Hand */}
                    <div className="absolute flex w-1 h-28 top-4 origin-bottom z-[3] rounded-full" style={{ transform: `rotate(${second * 6}deg)` }}>
